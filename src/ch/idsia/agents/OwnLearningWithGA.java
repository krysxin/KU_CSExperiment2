...
public class OwnLearningWithGA implements LearningAgent {
	...
	final float mutateRate = 0.2f; // 突然変異率
	final float crossRate = 0.5f; // 交差率
	...
	protected EvaluationFunc upper_eval = (agent, info) -> info.distancePassedCells;
	protected EvaluationFunc under_eval = 0;
	protected EvaluationFunc default_eval = 0;
	protected EvaluationFunc[] islands = new EvaluationFunc[] { upper_eval, under_eval, default_eval, default_eval,
			default_eval };
	public OwnGAAgent[][] agents;
	public OwnGAAgent[][] temp_agents;
	private double ex_mutate_rate = 0.45;
	private double in_mutate_rate = 0.2f;
	/* 評価時最大値保持用変数 */
	float fmax;
	...
	public OwnLearningWithGA(String args) {
		...
		/* 個体数分だけAgentを作成 */
		agents = new OwnGAAgent[islands.length][popsize];
		temp_agents = new OwnGAAgent[islands.length][popsize - bestnum];
		for (int n = 0; n < islands.length; n++) {
			for (int i = 0; i < popsize; i++) {
				agents[n][i] = new OwnGAAgent();
			}
			for (int i = 0; i < popsize - bestnum; i++) {
				agents[n][i] = new OwnGAAgent();
			}
		}

		/* agent[0] をbestAgentとして初期化 */
		bestAgent = agents[0][0].clone();
		this.args = args;
	...
	public void learn() {
		 ...
		 OwnGAAgent.loadLearningGene(agents, args);
		...
		// OwnGAAgent.saveLearningGene(agents, args);
	...
	private void compFit() {
		...
		for (int i = 0; i < popsize; i++) {
			...
			agents[i].setFitness(evaluationInfo.distancePassedCells); // この値を変える
	...
	private int choiceParent(OwnGAAgent[] island) {
		int eval_sum = 0;
		for (int n =0 ; n<popsize; n++) {
			eval_sum += island[n].getEvaluaiton();
		}
		int target = (int) (eval_sum * Math.random());
		for (int n =0 ; n<popsize; n++) {
			if((target -= island[n].getEvaluaiton())<0) {
				return n;
			}
		}
		return -1;
	...
	// EvaluationFun interface
	interface EvaluationFunc {
		public int evaluate(OwnGAAgent agent, EvaluationInfo info); // 抽象化メソッド
	} // evaluate を object化するため
